...
{
    public abstract class AbpMpaMvcEfInitTestBase : AbpIntegratedTestBase<AbpMpaMvcEfInitTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected AbpMpaMvcEfInitTestBase()
        {
        ...
        protected void UsingDbContext(Action<AbpMpaMvcEfInitDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<AbpMpaMvcEfInitDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<AbpMpaMvcEfInitDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<AbpMpaMvcEfInitDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<AbpMpaMvcEfInitDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpMpaMvcEfInitDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<AbpMpaMvcEfInitDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<AbpMpaMvcEfInitDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<AbpMpaMvcEfInitDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<AbpMpaMvcEfInitDbContext, Task<T>> func)
        {
            T result;
